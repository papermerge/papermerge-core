...
def get_group(db_session: Session, group_id: int) -> schemas.GroupDetails:
    with db_session as session:
        stmt = (
            select(models.Group)
            .options(joinedload(models.Group.permissions))
        ...
        result = schemas.GroupDetails.model_validate(db_item)

    return result


def get_groups(db_session: Session) -> list[schemas.Group]:
    with db_session as session:
        stmt = select(models.Group)
        db_items = session.scalars(stmt).all()
        result = [schemas.Group.model_validate(db_item) for db_item in db_items]
...
def create_group(
    ...
    with db_session as session:
        ...
        stmt = select(models.Permission).where(models.Permission.codename.in_(scopes))
        perms = session.execute(stmt).scalars().all()
        group = models.Group(name=name, permissions=perms)
        session.add(group)
        session.commit()
...
def update_group(
    db_session: Session, group_id: int, attrs: schemas.UpdateGroup
) -> schemas.Group:
    with db_session as session:
        ...
        session.commit()
        result = schemas.Group(id=group.id, name=group.name)
