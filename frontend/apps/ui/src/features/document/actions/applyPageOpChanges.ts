...
>(
  "document/applyPageChanges",
  async ({docID, pages, mode}, {getState, dispatch}) => {
    ...
    // Call RTK Query mutation manually
    ...
    if ("error" in result) {
      throw result.error // propagates error to rejected action
    }
