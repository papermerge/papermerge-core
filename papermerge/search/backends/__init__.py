...
from papermerge.search.search_result import SearchResult
from papermerge.search.utils import get_model_ct
from papermerge.search.utils.loading import UnifiedIndex
...
class BaseSearchBackend:
    ...
    def more_like_this(
        ...
        """
        raise NotImplementedError(
            "Subclasses must provide a way to fetch similar record"
            "   via the 'more_like_this' method if supported by the backend."
        )
    ...
    def extract_file_contents(self, file_obj):
        ...
        raise NotImplementedError(
            "Subclasses must provide a way to extract metadata"
            " via the 'extract' method if supported by the backend."
        )
    ...
    def build_models_list(self):
        ...
        for model in (
            connections.conn.get_unified_index().get_indexed_models()
        ):
...
class SearchNode(tree.Node):
    ...
    def __str__(self):
        ...
        return "(%s: %s)" % (self.connector, ", ".join([
...
class BaseSearchQuery:
    ...
    ``build_query_fragment``, ``clean`` and ``run``. See the ``solr``
    ...
    def run_mlt(self, **kwargs):
        ...
        if self._more_like_this is False or self._mlt_instance is None:
            raise MoreLikeThisError(
                "No instance was provided to "
                "determine 'More Like This' results."
            )
    ...
    def build_query(self):
        ...
        """
        final_query = self.query_filter.as_query_string(
    ...
    def build_query_fragment(self, field, filter_type, value):
        ...
        """
        raise NotImplementedError(
            "Subclasses must provide a way to generate query fragments via "
            "the 'build_query_fragment' method."
        )
    ...
    def add_field_facet(self, field, **options):
        ...
        field_name = (
            connections.get_unified_index().get_facet_fieldname(field)
        )
    ...
    def add_date_facet(self, field, start_date, end_date, gap_by, gap_amount=1):
        ...
        self.date_facets[
            connections.conn.get_unified_index().get_facet_fieldname(field)
        ] = details
    ...
    def post_process_facets(self, results):
        ...
        revised_facets = {}
        field_data = connections.conn.get_unified_index().all_searchfields()
    ...
    def get_unified_index(self):
        if self._index is None:
            self._index = self.unified_index(
                self.options.get("EXCLUDED_INDEXES", [])
            )
