...
from papermerge.search.exceptions import NotHandled
...
class SearchQuerySet:
    ...
    def _determine_backend(self):
        ...
        self.query = connections.conn.get_query()
    ...
    def __repr__(self):
        return "<SearchQuerySet: query=%r, using=%r>" % (
            self.query,
            self._using
        )
    ...
    def __len__(self):
        ...
        # This needs to return the actual number of hits, not what's in
        # the cache.
    ...
    def post_process_results(self, results):
        ...
        if self._load_all:
            ...
            # Remember the search position for each result so we don't have
            # to resort later.
        ...
        for result in results:
            if self._load_all:
                ...
                # We have to deal with semi-arbitrary keys being cast from
                # strings (UUID, int, etc)
                if model_objects:
                    ...
                    try:
                        ...
                        # The object was either deleted since we
                        # indexed or should
                        # be ignored for other reasons such as an
                        # overriden 'load_all_queryset';
                        # fail silently.
                    ...
                    # No objects were returned -- possible due to SQS nesting
                    # such as
    ...
    def _fill_cache(self, start, end, **kwargs):
        ...
        while True:
            ...
            # Assign by slice.
            self._result_cache[
                cache_start: cache_start + len(to_cache)
            ] = to_cache
    ...
    def __getitem__(self, k):
        ...
        if len(self._result_cache) <= 0 or (
            None in self._result_cache[start:bound] and
            not self._cache_is_full()
        ):
            try:
    ...
    def models(self, *models):
        ...
        for model in models:
            if (
                ...
                logger.warning(
                    "The model %r is not registered for search." % (model,)
                )
    ...
    def values_list(self, *fields, **kwargs):
        ...
        if flat and len(fields) > 1:
            raise TypeError(
                "'flat' is not valid when values_list"
...
class ValuesSearchQuerySet(ValuesListSearchQuerySet):
    ...
    def _fill_cache(self, start, end):
        ...
        query_fields.update(self._fields)
        kwargs = {"fields": query_fields}
        return super(ValuesListSearchQuerySet, self)._fill_cache(
            start, end, **kwargs
        )
...
class RelatedSearchQuerySet(SearchQuerySet):
    ...
    def load_all_queryset(self, model, queryset):
        """
        Allows for specifying a custom ``QuerySet`` that changes how
        ``load_all``
